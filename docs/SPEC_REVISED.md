...
#### 3.0 Recommended Visualization Workflow & User Progression

To maximize the pedagogical value in STEM classrooms, the 3D viewport operates on a structured, sequential workflow designed to guide the student's learning loop:

| Stage | View Mode | Timing & Trigger | Visualization Specifications | Pedagogical Objective |
| :--- | :--- | :--- | :--- | :--- |
| **1** | **3D STL View** | Raw import; on STL file upload | Raw 3D mesh geometry, shaded gray with active grid floor, wireframe toggle, and scale bounding dimensions. | Verify import scaling (e.g. mm-to-meters correction), watertightness confidence, and correct flow direction alignment (+X axis). |
| **2** | **Pressure Map** | Immediate simulation completion | WebGL-shaded surface heat map mapping pressure ranges: stagnation (red) to separation wake (deep blue). | Connect local geometry shapes directly to aerodynamic forces (e.g. flat windshield = red stagnation zone). |
| **3** | **Streamlines** | Secondary analysis selection | Animated particle flow lines wrapping around the body, color-coded by local wind velocity magnitude. | Understand kinetic wind behavior, fluid split angles, and wake/turbulence separation. |
| **4** | **Slice View** | Detailed diagnostic check | Interactive cross-sectional slicing plane (X, Y, Z axes) with 2D velocity vectors. | Pinpoint exact boundary layer detachment coordinates to target specific CAD revisions in Onshape. |
#### 3.0.1 Technical Implementation Recommendations

To ensure an interactive, high-fidelity experience that runs reliably on school hardware (including Chromebooks), the visualization engine inside the 3D viewport is built using **Three.js** with the following optimized WebGL pipeline:

1. **Stage 1 (3D STL View) - Import Verification & Centering**:
   - **Ingestion**: Serve the raw `.stl` file from a dedicated backend endpoint (`/api/designs/{design_id}/stl`) and load it via `THREE.STLLoader`.
   - **Centering & Normalization**: Auto-center the mesh using `geometry.center()` to establish an origin of `(0,0,0)`. Measure the model's bounding box and scale it dynamically to fit a standardized viewport coordinate frame (e.g. baseline length of 3 to 4 units in Three.js).
   - **Environment**: Draw a high-contrast reflective gray material (`MeshStandardMaterial` with `roughness: 0.4`, `metalness: 0.7`), soft directional lights, ambient background gradients, and an active helper grid on the floor.

2. **Stage 2 (Pressure Map) - Dynamic Surface Force Mapping**:
   - **Computation**: High/low-pressure zones are mapped using the dot product of the surface normals with the wind flow axis. Since the wind flows lengthwise along the X-axis:
     - Vertices with normals pointing forward (`nx > 0.1`) represent high-pressure stagnation zones.
     - Vertices with normals pointing backward (`nx < -0.1`) represent low-pressure separation zones in the rear wake.
     - Vertices parallel to the flow (`nx` near `0.0`) represent low-friction friction boundaries.
   - **Color Mapping**: Map the normal's X component (`nx`) dynamically to a premium HSL rainbow color spectrum (deep blue/cyan for low pressure $\rightarrow$ green/yellow for ambient $\rightarrow$ vibrant red/magenta for stagnation pressure).
   - **Implementation**: Set these colors directly into `geometry.setAttribute('color', new THREE.Float32BufferAttribute(colors, 3))` using `MeshStandardMaterial` with `vertexColors: true`. This runs client-side instantly with zero GPU compilation lag, preventing rendering errors on lower-end school hardware.

3. **Stage 3 (Streamlines) - Organic Vector Particle Flow**:
   - **Streamline Curves**: Generate 30 custom streamline paths. Streamlines are calculated as smooth curving ribbons starting in a grid configuration in front of the vehicle.
   - **Fluid Deflection**: As path points step forward along the flow axis (X-axis), deflect their transverse coordinates (Y and Z) outwards if they approach the vehicle's bounding volume. Use an analytical potential-flow repeller formula centered at the car's bounding box center:
     $$\vec{r}_{deflected} = \vec{r}_{current} + \frac{\alpha \cdot \vec{d}}{d^3}$$
     This creates highly realistic fluid contours that wrap around any arbitrary car shape.
   - **Particle Flow Animation**: Animate a set of bright glowing white spheres flowing along these computed paths using a fast `requestAnimationFrame` loop, representing kinetic wind speed.

4. **Stage 4 (Slice View) - Lengthwise Clipping Plane**:
   - **Clipping**: Activate a local clipping plane perpendicular to the lengthwise axis (X-axis) using `THREE.Plane` and `renderer.localClippingEnabled = true`.
   - **Interactive Slide**: Connect a range slider in the UI to the plane's constant, letting students slice through the car's 3D geometry in real-time.
   - **Pedagogical Feedback**: Render a colored flat cap at the intersection plane to clearly outline the vehicle's interior cross-section.
